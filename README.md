...
Проект включает локальный Docker Compose-профиль, Helm chart, values-файлы для окружений `test` и `prod`, pipeline для проверки, сборки Docker-образов, публикации образов в registry и развёртывания приложения в Kubernetes.

Прикладной сценарий используется как проверяемая бизнес-нагрузка для деплоя: пользователь регистрируется, получает каталог товаров, создаёт заказ, получает PDF-счёт, выполняет оплату, получает PDF-чек и email-уведомление.
